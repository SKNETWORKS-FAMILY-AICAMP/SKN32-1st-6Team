...
## 📊 수집 데이터 정리 
본 프로젝트는 자동차 등록 현황 분석, 대기업 FAQ 시스템 구축, 전국 정비소 조회 서비스를 위해 다양한 데이터를 수집 및 활용하였습니다. 

### 1. 📈 시도별 자동차 등록 현황 데이터
* 출처: KOSIS 자동차등록현황
* 수집 방법: 엑셀 파일 다운로드 후 Python 자동 파싱
* 수집 범위: 전국 시도 및 시군구
* 주요 데이터
  * 시도별 분류 
  * 차종별 분류
...
### 2. 🛠️ 전국 자동차 정비업체 표준 데이터 
* 출처: 공공데이터포털
* 수집 방법: CSV 파일 다운로드 및 시스템 자동 적재
* 수집 범위: 전국
* 주요 데이터
  * 자동차정비업체명
  * 자동차정비업체종류
...
### 3. 🚗 현대/기아자동차 FAQ 데이터 
* 출처: 현대자동차 고객센터 / 기아 고객센터
* 수집 방법: Selenium + BeautifulSoup 기반 웹 크롤링
* 저장 방식: MySQL DB 연동
* 주요 데이터
  * 회사 브랜드
  * 카테고리
...
데이터 활용 수집된 데이터는 전처리 및 DB 적재 과정을 거쳐 시스템과 연동되었으며, 사용자는 지역·차종·브랜드 조건에 따라 차량 통계 조회, FAQ 검색, 정비소 조회 기능을 사용할 수 있도록 구현하였습니다.
...
- 막대 차트, 파이 차트, 스택 차트
- 시군구 상세 조회
![image](./images/car_page_1.png)
![image](./images/car_page_2.png)
![image](./images/car_page_3.png)
![image](./images/car_page_4.png)
...
![image](./images/FAQ_page_1.png)
![image](./images/FAQ_page_2.png)
...
- 도 -> 시/군 -> 구 지역 계층별 조회
- 검색어 기반 사용자 지정 서칭 가능  
![image](./images/repairshop_page_1.png)
...
<table>
  ...
  <tr>
    <td align="center">김남균</td>
    <td> 
    첫 프로젝트의 막막함을 팀원들과의 체계적인 협업으로 극복하며 완수했습니다. 특히 GitHub 기반의 코드 통합 과정에서 다양한 충돌을 함께 해결하며 실무 트러블 슈팅 능력을 키운 값진 경험이었습니다.
    </td>
  </tr>
  
  <tr>
    <td align="center">서유현</td>
    <td> 
    첫 프로젝트이다 보니 진행 과정에서 모든 팀원의 아이디어를 충분히 반영하지 못한 점이 아쉬웠다. 아직 배우는 단계라 질문과 오류가 많아 팀원들에게 미안한 마음도 컸지만, 이번 프로젝트를 통해 많은 것을 배우고 좋은 경험을 쌓을 수 있었다. 도움 주신 팀원분들께 다시 한 번 감사드린다.
    </td>
  </tr>
  
  <tr>
    <td align="center">송채영</td>
    <td> 
      수업시간에 배운 내용을 활용하여 프로젝트를 만들 수 있어 좋았고, 프로젝트 경험이 많이 없고 관련 지식도 부족해서 걱정을 했지만 팀원들과의 협업으로 잘 마무리할 수 있었다. 프로젝트를 통해 다양한 지식을 알게 되었고, 지금보다 더 많이 공부해야겠다는 생각이 들었다. 이번에 느꼈던 부족함을 다음 프로젝트 때 보완하는 것을 작은 목표로 삼고, 함께 해주신 팀원분들께 감사의 인사를 전한다. 
    </td>
  </tr>
  
  <tr>
    <td align="center">임정택</td>
    <td> 
    완벽하고 좋은 프로젝트보다는 팀원들이 모두 함께 성장해 나갈 수 있도록 하려고 노력했습니다. 어느 정도 그런 목표를 달성한 것 같아서 보람이 있었습니다.
    </td>
  </tr>
