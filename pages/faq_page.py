...
[크롤러 라우팅 규칙]
  company_code == 'hyundai'  →  crawling/hyundai_faq_crawling.py  run()  (합친 코드)
  그 외                      →  crawlers/kia_crawler.py  crawl_generic_faq()  (범용)
"""
...
def render():
    _init_state()
    st.image("images/기업FAQ.png", use_container_width=True)
    st.header("🔍 기업 FAQ 조회")
...
def _render_crawl_tab():
    ...
    # 현대자동차 전용 크롤러 안내
    if selected["company_code"] == "hyundai":
        st.success(
            "✅ **현대자동차 전용 크롤러 연결됨**\n\n"
            "`crawling/hyundai_faq_crawling.py` → `run()` 함수가 실행됩니다.\n"
            "탭별 전체 페이지를 순회하며 수집하고, DB 및 CSV(`data/hyundai_faq.csv`)에 저장합니다.",
            icon="🚗"
        )
    ...
    with col2:
        if st.session_state.crawl_running:
            st.info("⏳ 크롤링 중... (브라우저가 자동으로 동작합니다)")
...
def _crawl_hyundai(log, session_id, max_items=None):
    ...
    완료 후 저장된 데이터를 읽어 Streamlit 결과 목록으로 반환합니다.
    """
    log("현대자동차 전용 크롤러 코드 실행 중...")
    ...
    # run()이 DB에 '현대자동차' 레코드를 저장하므로 여기서 읽어 반환
    results = []
    try:
        from model.faq_model import FAQModel
        fm = FAQModel()
        rows = fm.select_all()
        # rows: (id, company_code, category, question, answer, crawled_at, session_id)
        for row in rows:
            results.append({
                "company":  row[1],
                "category": row[2],
        ...
        fm.close()
        log(f"DB에서 {len(results)}개 데이터 로드 완료")
    except Exception as e:
        log(f"⚠️ DB 결과 로드 실패 (CSV로 대체): {e}")
        # DB 실패 시 크롤러가 저장한 CSV에서 읽기
        csv_path = ROOT / "data" / "hyundai_faq.csv"
        if not csv_path.exists():
            csv_path = ROOT / "hyundai_faq.csv"
        if csv_path.exists():
            df = pd.read_csv(csv_path, encoding="utf-8-sig")
            results = df.to_dict("records")
            log(f"CSV에서 {len(results)}개 데이터 로드 완료")
...
def _render_accumulated_tab():
    ...
    # DB 없을 때 현대자동차 CSV 대체 조회
    if not DB_AVAILABLE:
        st.warning("MySQL 미연결 상태입니다. 현대자동차의 경우 CSV 파일로 조회할 수 있습니다.")
        csv_path = ROOT / "data" / "hyundai_faq.csv"
        if csv_path.exists():
            df = pd.read_csv(csv_path, encoding="utf-8-sig")
            st.info(f"📄 `data/hyundai_faq.csv` — {len(df)}개 레코드")
            search_csv = st.text_input("🔍 키워드 검색 (CSV)", key="csv_search")
            if search_csv:
                mask = df.apply(lambda r: r.astype(str).str.contains(search_csv, na=False).any(), axis=1)
                df = df[mask]
            st.dataframe(df, use_container_width=True, height=350, hide_index=True)
            for _, row in df.head(30).iterrows():
                with st.expander(f"❓ {str(row.get('question',''))[:80]}"):
                    st.caption(f"📂 {row.get('category','')}")
                    st.markdown(f"**Q.** {row.get('question','')}")
                    st.markdown(f"**A.** {row.get('answer','(답변 없음)')}")
        else:
            st.error("MySQL 연결이 필요합니다. 현대자동차 크롤링 후 CSV도 자동 생성됩니다.")
        return
